...
public class ADConnect {
	...
	public static void main(String[] args) {
		...
		String ntfy_s_fltr = "(objectClass=user)";
		
		String search_base = ConfigurationHelper.all_search_bases();
		//String search_base = "OU=SyncplicityUsers,DC=support12r2,DC=com";
		
		String ntfy_srch_base = ConfigurationHelper.all_search_bases();
		//String search_base = ConfigurationHelper.all_search_bases();
		//String ntfy_srch_base = "OU=SyncplicityUsers,DC=support12r2,DC=com";
		...
		ADConnect adc_ntfy = new ADConnect();
		adc.getNotifyData(adc_ntfy.createContext(url, usr, pas), ntfy_srch_base, ntfy_s_fltr);
	...
	public List<AdUser> getNotifyData(LdapContext ctx, String sBase, String sFltr) throws NamingException
	{
	...
	try
	{
            ...
            ctx_ntfy.setRequestControls(rqstCtls);
			...
			//Search for objects using the filter
			NamingEnumeration answer = ctx_ntfy.search(searchBase, searchFltr, searchCtrls);
			...
			while (answer.hasMoreElements())
			{
				...
				SearchResult s_rslts = (SearchResult)answer.next();
				Attributes ntfy_attrs = s_rslts.getAttributes();
				
				//System.out.println(results_tot+1 + " ->>>" + s_rslts.getName());
				System.out.println(">>>" + s_rslts.getName());
						
					if (ntfy_attrs != null)
						try {

                            for (NamingEnumeration<? extends Attribute> ae = ntfy_attrs.getAll();ae.hasMore();) {
